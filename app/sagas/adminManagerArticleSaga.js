/**
 * Created by vijay on 2018/2/23.
 */
import {take, call, put, select} from 'redux-saga/effects';
import {get} from '../fetch/fetch';
import {actionsTypes as IndexActionTypes} from '../reducers';
import {actionTypes as ArticleTypes} from '../reducers/adminManagerArticle';
import {actionTypes as NewArticleTypes} from '../reducers/adminNewArticle';
...
export function* getArticleListFlow() {
    while (true) {
        ...
        // console.log("adminManagerArticleSage, getArticleListFlow, res: ", res);
        if (res) {
            if (res.code === 0) {
...
export function* deleteArticle(id) {
    ...
    try {
        ...
        yield put({type: IndexActionTypes.SET_MESSAGE, msgContent: '网络请求出错', msgType: 0});
    } finally {
        yield put({type: IndexActionTypes.FETCH_END});
    }
}
...
export function* deleteArticleFlow() {
    while(true) {
        ...
        console.log("adminManagerArticle deleteArticleFlow, res: ", res);
        if (res) {
            if(res.code === 0) {
